...
export default class RevitalizerPresenter extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    #hasUnrevitalizableReason(data, revitalizableProperties) {
        if (!getSettings(settings.revitalize.id))
            return game.i18n.localize("PIR.presented.disabled_in_settings");

        if (revitalizableProperties.length === 0)
            return game.i18n.localize("PIR.presented.revitalize_will_not_recreate");

        if (this.#sourceIsBestiary(data.actorItem.sourceId))
            return game.i18n.localize("PIR.presented.bestiary_warning");
    ...
    #getButtons(data) {
        ...
        return {
            "refresh": {
                disabled: isRefreshable ? false : (game.i18n.localize("PIR.presented.refresh_not_available") + (getSettings(settings.revitalize.id) ? "" : game.i18n.localize("PIR.presented.refresh_not_available_revitalize_hint"))),
                hidden: !isRefreshable && isRevitalizable,
                icon: "fa-solid fa-sync-alt",
                action: "refresh",
                value: data.actorItem.uuid,
                title: game.i18n.localize("PIR.presented.refresh_title")
            },
            "revitalize": {
                ...
                title: `${game.i18n.localize("PIR.presented.revitalize_title_prefix")} ${revitalizableProperties.join(", ")}`
            },
            "hide": {
                icon: "fa-regular fa-eye-slash",
                action: "hide",
                value: data.actorItem.uuid,
                title: game.i18n.localize("PIR.presented.hide_title")
            },
            "remove": {
                icon: "fa-regular fa-check",
                action: "remove",
                value: '',
                title: game.i18n.localize("PIR.presented.remove_title")
            },
        }
    }
    ...
    async present(changedData, actors) {
        popup(game.i18n.localize("PIR.presented.parsing_complete"));
    ...
    close() {
        super.close();
        popup(game.i18n.localize("PIR.presented.reload_application"), { permanent: true, console: false });
    }
    ...
    static async revitalize(event, target) {
        ...
        try {
            ...
            console.error(error);
            popup(game.i18n.localize("PIR.presented.error_find_uuid"));
            return false;
        }
        properties.forEach(property => {
            try {
                ...
                console.error(error);
                popup(game.i18n.format("PIR.presented.error_revitalize_property", { property }));
                return false;
            }
        });
        popup(game.i18n.format("PIR.presented.item_success_revitalized", { name: actorItem.slug }));
    ...
    static async refreshFromCompendium(event, target) {
        ...
        if (actorItem.name.includes("(At Will)")) {
            replaceName = false;
            popup(game.i18n.format("PIR.presented.will_not_rename", { name: actorItem.name }));
        }
